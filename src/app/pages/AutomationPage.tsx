...
// Khởi tạo Supabase giống hệt bên file Audit Log
...
export default function AutomationPage() {
  ...
  // 🌟 THÊM: Hàm ghi Audit Log dùng chung
  const logActionToAudit = (actionName: string, resourceName: string) => {
    const currentLogs = JSON.parse(localStorage.getItem('auditLogs') || '[]');
    const userEmail = localStorage.getItem('userEmail') || 'Phan Huỳnh Sa Linh';
    ...
    localStorage.setItem('auditLogs', JSON.stringify([newLog, ...currentLogs]));
  };
  ...
  useEffect(() => {
    ...
    if (newWorkflowData) {
      ...
      // 🌟 THÊM: Ghi log khi Workflow thực sự được tạo ra và lưu thành công
      logActionToAudit('Created New Workflow', newWorkflow.name);
  ...
  const handleNewWorkflow = () => {
    // 🌟 THÊM: Ghi log khi bắt đầu bấm nút tạo mới
    logActionToAudit('Initiated Workflow Creation', 'New Draft');
    navigate('/builder');
  };
  ...
  const handleWorkflowClick = (workflow: Workflow) => {
    // 🌟 THÊM: Ghi log khi click vào sửa (Edit/Cập nhật)
    logActionToAudit('Opened Workflow for Edit', workflow.name);
      ...
      <div className="flex h-[calc(100vh-64px)]">
        ...
        <main className="flex-1 overflow-y-auto">
          <div className="p-8">
            {/* Header */}
            <div className="mb-6">
              <h1 className="text-2xl font-semibold text-gray-900 mb-4">Automation</h1>
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-2">
                            <button 
                              onClick={(e) => {
                                e.stopPropagation();
                                // 🌟 THÊM: Ghi log khi click chạy thử
                                logActionToAudit('Manually Ran Workflow', workflow.name);
                                alert(`Đã chạy thử: ${workflow.name}`);
                              }}
